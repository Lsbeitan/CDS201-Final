;; models the transference of colorism in two directions, cpolice learn from police and police learn from cpolice.
;; Written in NetLogo
;; Author: Layan Sbeitan and Fatma Abdulwahab
;; Creation Date: November 23, 2018
;; Code taken from Henry Smart's Local Policing and Colorism Netlogo Model

;; a cpolice with a convert of -2 at the end of the simulation denotes a converted police.
;; a police with a convert of 2 at the end of the simulation denotes a converted cpolice.
;; when two like officers cross paths, there is a headnod-affirmation/no change in the convert count.
...
globals [
  ...
  incarceration-patches    ;; space, group of patches that represent the incarceration area for juveniles-agents tried as adults
  juveniledetention-patches ;; space, group of patches that represent the incarceration area for citizen-agents tried as juveniles and are admitted into detainment centers
...
;;police agents
breed [mpolices mpolice]     ;;police officers that are Black (lights)
breed [cpolices cpolice]   ;;police officers that are Latino (mediums)
breed [polices police]     ;;police officers that are White (lights)
...
to setup
  ...
  ;; create free-patches
  set free-patches patches with [pycor >= 1 and pycor <= 40]
  ask free-patches [ set pcolor white ]

  ;; create detainment-patches
  set detainment-patches patches with [pycor >= -10 and pycor <= 1]
  ask detainment-patches [ set pcolor gray ]

  ;; create incarceration-patches
  set incarceration-patches patches with [pycor >= -30 and pycor <= -10 and pxcor >= -30 and pxcor <= 1 ]
  ask incarceration-patches [ set pcolor red ]

  ;; create juveniledetention-patches
  set juveniledetention-patches patches with [pycor >= -30 and pycor <= -10 and pxcor >= 1 and pxcor <= 30 ]
  ask juveniledetention-patches [ set pcolor orange ]
  ...
  create-cpolices #-of-cpolice [     ;;creates police-agents with light skin color
    set shape "police"
    set size 1
    set color yellow
  ...
  create-polices #-of-police [       ;;creates police-agents with medium skin color
    set shape "police"
    set size 1
    set color brown
  ...
  create-mpolices #-of-mpolice [       ;;creates police-agents with dark skin color
    set shape "police"
    set size 1
    set color black
...
to go
  ...
  ask mpolices [                                                                     ;;starts mpolicing of the free-patches
    move-to one-of free-patches
    ]
  ...
  ask darks                                                                         ;;procedure for detainment of darks by police and intiation of rapsheet tally
    with[pcolor = white] [
    if any? turtles with [shape = "police" and color = yellow and pcolor = white]
    in-radius 1
    ...
    ask mediums                                                                     ;;procedure for detainment of mediums by police and intiation of rapsheet tally
      ...
      if any? turtles with [shape = "police" and color = yellow and pcolor = white]
      in-radius 1
      [move-to one-of detainment-patches set rap rap + 1.5]
      ]
  ...
  ask darks                                                                         ;;procedure for detainment of darks by cpolice and intiation of rapsheet tally
    with[pcolor = white] [
    if any? turtles with [shape = "police" and color = brown and pcolor = white]
    in-radius 1
    [move-to one-of detainment-patches set rap rap + 1.5]
    ]
   ...
   ask darks                                                                         ;;procedure for detainment of darks by lpolice and intiation of rapsheet tally
    with[pcolor = white] [
    if any? turtles with [shape = "police" and color = black and pcolor = white]
    in-radius 1
    [move-to one-of detainment-patches set rap rap + 1]
    ]


    ask mediums                                                                     ;;procedure for detainment of mediums by lpolice and intiation of rapsheet tally
      with[pcolor = white] [
      if any? turtles with [shape = "police" and color = black and pcolor = white]
      in-radius 1
      [move-to one-of detainment-patches set rap rap + 1.5]
    ...
    ask lights                                                                      ;;procedure for detainment of lights by lpolice and intiation of rapsheet tally
      with[pcolor = white] [
      if any? turtles with [shape = "police" and color = black and pcolor = white]
      in-radius 1
      [move-to one-of detainment-patches set rap rap + 1.5]
  ...
  ask turtles                                                                       ;;returns detained citizen-agents to free-society
    with [pcolor = gray and shape = "person"][
  ...
  ask turtles                                                                       ;;incarcerates citizen-agents based on a three-strike rule
    ...
    if rap >= 4 and rap < 5 and color = black [
    move-to one-of juveniledetention-patches]

    if rap >= 5 and color = black [
    move-to one-of incarceration-patches]
    ]

  ask turtles                                                                       ;;incarcerates citizen-agents based on a three-strike rule
    with [pcolor = white and shape = "person"][
    if rap >= 6 and rap < 8 and color = brown [
    move-to one-of juveniledetention-patches]

    if rap >= 8 and color = brown [
    move-to one-of incarceration-patches]
    ]

  ask turtles                                                                       ;;incarcerates citizen-agents based on a three-strike rule
    with [pcolor = white and shape = "person"][
    if rap >= 8 and rap < 10 and color = yellow [
    move-to one-of juveniledetention-patches]

    if rap >= 10 and color = yellow [
    move-to one-of incarceration-patches]
    ]
...
"Juvenile Detention" 1.0 0 -955883 true "" "plot count darks with [pcolor = orange]"
...
"Juvenile Detention Centers" 1.0 0 -955883 true "" "plot count mediums with[pcolor = orange]"
...
"Juvenile Detention Centers" 1.0 0 -955883 true "" "plot count lights with[pcolor = orange]"
...
371
Blacks in Juvie
count darks with [pcolor = orange]
17
1
11
...
384
373
Blacks in Adult Prisons
count darks with [pcolor = red]
17
1
11
...
431
Latinos in Juvie
count mediums with [pcolor = orange]
17
1
11
...
385
431
Latinos in Adult Prisons
count mediums with [pcolor = red]
17
1
11
...
491
# Free Whites
count lights with [pcolor = white]
17
1
11
...
491
White in Juvie
count lights with [pcolor = orange]
17
1
11
...
382
492
White in Adult Prisons
count lights with [pcolor = red]
17
1
11
...
430
# Free Latinos
count mediums with [pcolor = white]
17
1
11
...
In this second version of the model, the rules are intended to simulate bi-directional influence. At each tick, a (fair) police checks to see if there is a c-police within close proximity (1 patch/cell).  If this condition is true, the police's convert counter is decreased by 1.  Once a (fair) police's convert counter reaches -2, their decision-making model will converted to that of a (biased) c-police.  At each tick, a (biased) police officer checks to see if there is a police within close proximity (1 patch/cell). If this condition is true, the cpolice's convert counter is increased by 1. Once the a c-police's convert counter reaches +2, their decision-making model will convert to that of a (fair) police.
...
<experiments>
  ...
  <experiment name="Logic 2 - More CPolice than Police" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>if all? darks [pcolor = red] [stop]</final>
    <timeLimit steps="1500"/>
  ...
  <experiment name="Logic 4 - Total Biased Policing" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>if all? darks [pcolor = red] [stop]</final>
    <timeLimit steps="1500"/>
  ...
  <experiment name="Logic 5 - Total Fair Policing" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>if all? darks [pcolor = red] [stop]</final>
    <timeLimit steps="1500"/>
  ...
  <experiment name="Logic 6 - U.S. Population - Equal Number of CPolice and Police" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>if all? darks [pcolor = red] [stop]</final>
    <timeLimit steps="1500"/>
  ...
  <experiment name="Logic 8 - U.S. Population - More Police than CPolice" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>if all? darks [pcolor = red] [stop]</final>
    <timeLimit steps="1500"/>
  ...
  <experiment name="Logic 10 - U.S. Population - Total Fair Policing" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <final>if all? darks [pcolor = red] [stop]</final>
    <timeLimit steps="1500"/>
